...
public class ArbolAVL {
 ...
 /* Verifica si el arbol esta vacio. 
    ...
    **/
    ...
    @Params: AAB: Nodo raiz del arbol.
    @Returns: AAB: Nueva raiz del arbol.
    **/
    public Nodo rebalancearEliminar(Nodo AAB) {
        actualizarAltura(AAB);
        int balanceado = balance(AAB);
        if (balanceado == 2) {
            if (altura(AAB.getHijoDer().getHijoDer()) > altura(AAB.getHijoIzq().getHijoIzq())) {
                AAB = rotarIzq(AAB);
            } else {
                AAB.setHijoDer(rotarDer(AAB.getHijoDer()));
                AAB = rotarIzq(AAB);

            }
        } else if (balanceado == -2) {
            if (altura(AAB.getHijoIzq().getHijoIzq()) > altura(AAB.getHijoIzq().getHijoDer())) {
                AAB = rotarDer(AAB);

            } else {
                AAB.setHijoIzq(rotarIzq(AAB.getHijoIzq()));
                AAB = rotarDer(AAB);
